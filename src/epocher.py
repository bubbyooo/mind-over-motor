...
class Data_Epoch:
    ...
    def build_logreg_dataset(self, root_dir, fs=500, seconds_per_trial=8.0, l_freq = 1.0, h_freq = 40.0, end = 4):
        """
        Discover and process all EDF files under root_dir.
        ...
        for subject_id, edf_path in enumerate(self.edf_files):
            ...
            # Used Claude for rest balancing
            n_rest = all_rest_trials.shape[0]
            n_keep = n_rest // 3 # Keep a random third of rest
            keep = torch.randperm(n_rest)[:n_keep]

            rest_trials = all_rest_trials[keep]
            rest_labels = torch.full((rest_trials.shape[0],), 2, dtype=torch.long) # label 2 = rest
